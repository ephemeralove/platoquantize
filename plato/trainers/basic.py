...
class Trainer(base.Trainer):
    ...
    def save_model(self, filename=None, location=None):
        ...
        if self.client_id == 0:
            logging.info("[Server #%d] Model saved to %s.",
                         os.getpid(), model_path)
        else:
            logging.info("[Client #%d] Model saved to %s.",
    ...
    def perform_forward_and_backward_passes(self, config, examples, labels):
        ...
        # examples, labels = examples.view(examples.size(0), -1), labels
    ...
    def train_model(self, config, trainset, sampler, **kwargs):
        ...
        for self.current_epoch in range(1, total_epochs + 1):
            ...
            self.callback_handler.call_event(
                "on_train_epoch_end", self, config)
    ...
    def test(self, testset, sampler=None, **kwargs) -> float:
        ...
        if hasattr(Config().trainer, "max_concurrency"):
            ...
            proc = mp.Process(
                target=self.test_process, args=(
                    config, testset, sampler), kwargs=kwargs
            )
            proc.start()
            proc.join()
